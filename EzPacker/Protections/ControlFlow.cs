    ...
    internal static class ControlFlow
    {
        ...
        internal static void Execute(ModuleDefMD Module)
        {
            ...
            foreach (TypeDef Type in Module.Types.Where(T => T.HasMethods))
                foreach (MethodDef Method in Type.Methods.Where(M => M.HasBody && M.Body.HasInstructions && M.Body.Instructions.Count() > 1 && !M.IsSetter && !M.IsGetter))
                {
                    if (Method == Module.GlobalType.FindOrCreateStaticConstructor()) continue;
                    if(Method.Name.StartsWith("DM"))
                    {
                        Method.Name = Method.Name.Substring(2);
                    } else
                    {
            ...
            if (controlflowed > 0)
                Console.WriteLine($"[CFlow] CFlowed {controlflowed} Methods !");
        }
